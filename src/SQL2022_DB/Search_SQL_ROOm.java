...
import Script.SearchSelectDocumentLocation;

public class Search_SQL_ROOm extends BaseClass{
   ...
   @Test(priority=4)
	...
	public void TextSearchWithSQLData() throws InterruptedException, IOException {
		...
		jsclick(sr.getFindButton());
		Thread.sleep(10000);
		jsclick(sr.getopendocfortextSQL());
		WebDriverWait wait = new WebDriverWait(driver, 30);
		try {
...
@Test (priority=12)// enter value in field and select the user
	...
	public void WorkflowStatusDropdown() throws InterruptedException {
       ...
       Thread.sleep(15000);
		...
		jsclick(sr.getforworkflowEndWorkflow());
		jsclick(sr.getFindButton());
		Thread.sleep(8000);
		jsclick(sr.getClearButton());
		Thread.sleep(8000);
